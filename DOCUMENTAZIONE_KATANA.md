...
- **Timeout Intelligente**: Evita blocchi su immagini complesse (max 10s per analisi colore)
- **Campionamento Adattivo**: Ottimizza l'analisi su immagini ad altissima risoluzione (>4000px)
- **Gestione Errori**: Recupero automatico con strategie di fallback

### 4. Quality Assurance
- **Test Automatici**: Script dedicati per metadati e autocrop
- **Check Quality**: Script unificato (`check_quality.bat`) per validazione rapida
- **Linting**: Verifica sintassi Python integrata

### 5. Interfaccia Grafica Avanzata
...
- **Barra Progresso**: Feedback visivo durante l'elaborazione
...
Katana offre una CLI completa. Esegui `python katana.py` senza argomenti per l'help.

```bash
# Esempio: elabora PDF in formato A4
python katana.py documento.pdf --format A4 --dpi 300

# Esempio: elabora intera directory
python katana.py . --no-crop
```

### Verifica e Test
Per assicurare la qualità del codice prima di modifiche:

```bash
# Esegue tutti i controlli di qualità (lint, test unitari, test integrazione)
check_quality.bat
```
