...
Katana offre una CLI completa per l'elaborazione dei documenti. Esegui `python katana.py` senza argomenti per visualizzare l'help.

```bash
python katana.py <input_file_or_dir> [opzioni]
```

**Opzioni:**
- `--output-dir DIR`: Directory di output (default: output_images)
- `--dpi DPI`: DPI target (default: 300)
- `--no-crop`: Disabilita ritaglio automatico
- `--format FMT`: Formato target (A4, A3, LETTER, LEGAL)
- `--test-ratio`: Esegui test rilevamento ratio

**Esempi:**

```bash
# Elaborazione di un singolo PDF formato A4
python katana.py documento.pdf --format A4
...
Per verificare l'integrità del sistema e le funzionalità core:

```bash
# Esegue compilazione, test metadati e test autocrop
check_quality.bat
```

### Ottimizzazioni Performance

Il sistema include ottimizzazioni automatiche per immagini di grandi dimensioni:
- **Analisi Colore Rapida**: Timeout intelligente (10s) per evitare blocchi
- **Campionamento Adattivo**: Riduce drasticamente i tempi su immagini >4000px
- **Ridimensionamento Ibrido**: Gestione intelligente di immagini fuori formato
```
...
Per dettagli completi, consultare il file [LICENSE](LICENSE).
