...
class KatanaProcessor:
    ...
    def __init__(self, output_dir: str = "output_images", learning_mode: bool = False, samurai_profile: str = None, auto_ratio_detection: bool = True):
        ...
        self.samurai_profiles_file = "katana_samurai_profiles.json"
        
        # Rilevamento automatico del ratio
        self.auto_ratio_detection = auto_ratio_detection
        
        # Rilevamento automatico dell'orientamento
        self.auto_orientation_detection = True
        
        # Sistema Samurai v3.0
        self.samurai_profiles = self._load_samurai_profiles()
        self.active_profile = samurai_profile or self.samurai_profiles.get('active_profile', 'documenti_vari')
        self.adaptive_params = self._get_profile_params(self.active_profile)
        
        # Mantieni compatibilità con v2.0
        if not self.samurai_profiles and os.path.exists(self.learning_file):
            self.adaptive_params = self._load_learning_data()
        
        # Log delle funzionalità attive
        logger.info(f"KatanaProcessor inizializzato - Output: {self.base_output_dir}")
        if learning_mode:
            logger.info("Modalità apprendimento ATTIVA")
        if samurai_profile:
            logger.info(f"Profilo Samurai attivo: {samurai_profile}")
        if auto_ratio_detection:
            logger.info("Rilevamento automatico del ratio ATTIVO - Katana riconoscerà automaticamente i formati delle immagini!")
        
    def extract_pdf_metadata(self, pdf_path: str, max_pages: int = None) -> Dict:
    ...
    def _load_samurai_profiles(self) -> Dict:
        """Carica i profili Samurai dal file JSON"""
        try:
            if os.path.exists(self.samurai_profiles_file):
                with open(self.samurai_profiles_file, 'r', encoding='utf-8') as f:
                    return json.load(f)
        except Exception as e:
            logger.warning(f"Errore nel caricamento profili Samurai: {e}")
        return {}
    
    def _get_profile_params(self, profile_name: str) -> Dict:
        """Ottiene i parametri adattivi per il profilo specificato"""
        if self.samurai_profiles and 'samurai_profiles' in self.samurai_profiles:
            profile = self.samurai_profiles['samurai_profiles'].get(profile_name)
            if profile:
                return profile['adaptive_params'].copy()
    ...
    def _detect_document_type(self, pdf_name: str) -> str:
        """Rileva automaticamente il tipo di documento dal nome del file"""
        pdf_name_lower = pdf_name.lower()
        
        for profile_name, profile_data in self.samurai_profiles.get('samurai_profiles', {}).items():
            patterns = profile_data.get('document_patterns', [])
            for pattern in patterns:
                if pattern.lower() in pdf_name_lower:
                    logger.info(f"Documento rilevato come {profile_name} (pattern: {pattern})")
                    return profile_name
        
        return 'documenti_vari'
    
    def set_active_profile(self, profile_name: str):
        """Imposta il profilo attivo"""
        if self.samurai_profiles and profile_name in self.samurai_profiles.get('samurai_profiles', {}):
            self.active_profile = profile_name
            self.adaptive_params = self._get_profile_params(profile_name)
            
            # Aggiorna il file dei profili
            self.samurai_profiles['active_profile'] = profile_name
            self._save_samurai_profiles()
            
            logger.info(f"Profilo attivo cambiato a: {profile_name}")
        else:
            logger.warning(f"Profilo {profile_name} non trovato")
    
    def get_available_profiles(self) -> Dict[str, Dict]:
        """Restituisce il dizionario dei profili disponibili"""
        profiles = {}
        if self.samurai_profiles and 'samurai_profiles' in self.samurai_profiles:
            for profile_name, profile_data in self.samurai_profiles['samurai_profiles'].items():
                profiles[profile_name] = {
                    'id': profile_name,
                    'name': profile_data['name'],
                    'description': profile_data['description'],
                    'accuracy_rate': profile_data['performance_stats']['accuracy_rate']
                }
        return profiles
    
    def enable_auto_ratio_detection(self):
        """Abilita il rilevamento automatico del ratio"""
        self.auto_ratio_detection = True
        logger.info("Rilevamento automatico del ratio ABILITATO")
    
    def disable_auto_ratio_detection(self):
        """Disabilita il rilevamento automatico del ratio"""
        self.auto_ratio_detection = False
        logger.info("Rilevamento automatico del ratio DISABILITATO")
    
    def enable_auto_orientation_detection(self):
        """Abilita il rilevamento automatico dell'orientamento"""
        self.auto_orientation_detection = True
        logger.info("Rilevamento automatico dell'orientamento ABILITATO")
    
    def disable_auto_orientation_detection(self):
        """Disabilita il rilevamento automatico dell'orientamento"""
        self.auto_orientation_detection = False
        logger.info("Rilevamento automatico dell'orientamento DISABILITATO")
    
    def detect_orientation(self, image):
        """Rileva l'orientamento corretto per un'immagine"""
        if not self.auto_orientation_detection:
            return 0
        
        try:
            # Converti l'immagine OpenCV in PIL se necessario
            if hasattr(image, 'shape'):  # È un array numpy/OpenCV
                import cv2
                from PIL import Image
            ...
            # Usa il metodo di rilevamento orientamento esistente
            orientation = self.detect_content_orientation(pil_image)
            
            # Mappa i risultati agli angoli di rotazione
            orientation_map = {
                'correct': 0,
                'rotate_90': 90,
                'rotate_180': 180,
            ...
            logger.error(f"Errore nel rilevamento orientamento: {e}")
            return 0
    
    def is_auto_ratio_detection_enabled(self) -> bool:
        """Verifica se il rilevamento automatico del ratio è abilitato"""
        return self.auto_ratio_detection
    
    def _save_samurai_profiles(self):
        """Salva i profili Samurai nel file JSON"""
        try:
            with open(self.samurai_profiles_file, 'w', encoding='utf-8') as f:
                json.dump(self.samurai_profiles, f, indent=2, ensure_ascii=False)
        except Exception as e:
            logger.error(f"Errore nel salvataggio profili Samurai: {e}")
    ...
    def save_feedback(self, pdf_name: str, page_num: int, feedback: str, bounds: Tuple[int, int, int, int] = None):
        ...
        feedback_entry = {
            ...
            'profile_used': self.active_profile
        }
        
        self.feedback_data.append(feedback_entry)
        
        # Sistema Samurai v3.0: aggiorna profilo specifico
        if self.samurai_profiles and 'samurai_profiles' in self.samurai_profiles:
            self._update_samurai_profile_feedback(feedback)
            self._save_samurai_profiles()
        else:
            # Compatibilità v2.0
        ...
        logger.info(f"Feedback salvato per {pdf_name} pagina {page_num}: {feedback} (profilo: {self.active_profile})")
    
    def _update_samurai_profile_feedback(self, feedback: str):
        """Aggiorna il feedback e le statistiche del profilo Samurai attivo"""
        if not self.samurai_profiles or 'samurai_profiles' not in self.samurai_profiles:
            return
            
        profile = self.samurai_profiles['samurai_profiles'].get(self.active_profile)
        if not profile:
            return
        ...
        if feedback == 'perfect':
            stats['perfect_feedback'] += 1
        elif feedback in ['good', 'light_cropping']:
            stats['good_feedback'] += 1
        else:
            stats['bad_feedback'] += 1
            
        # Calcola accuracy rate
        total_good = stats['perfect_feedback'] + stats['good_feedback']
        stats['accuracy_rate'] = round(total_good / stats['total_processed'], 3) if stats['total_processed'] > 0 else 0.0
        
        # Aggiorna parametri adattivi del profilo
        self._update_profile_adaptive_params(feedback)
        
        # Aggiorna timestamp
        profile['last_updated'] = datetime.now().isoformat()
        
        # Aggiungi feedback alla storia (mantieni ultimi 50)
        feedback_entry = {
            'timestamp': datetime.now().isoformat(),
            'feedback': feedback,
            'params_before': self.adaptive_params.copy()
        }
        ...
        logger.info(f"Profilo {self.active_profile} aggiornato - Accuracy: {stats['accuracy_rate']:.1%}")
    
    def _update_profile_adaptive_params(self, feedback: str):
        """Aggiorna i parametri adattivi del profilo attivo"""
        if feedback == 'excessive_cropping':
            self.adaptive_params['variance_threshold'] = max(20, self.adaptive_params['variance_threshold'] * 0.9)
            self.adaptive_params['entropy_threshold'] = max(0.8, self.adaptive_params['entropy_threshold'] * 0.9)
            self.adaptive_params['margin_adjustment'] = min(2.0, self.adaptive_params['margin_adjustment'] * 1.2)
            
        elif feedback in ['no_change', 'insufficient_cropping']:
            self.adaptive_params['variance_threshold'] = min(60, self.adaptive_params['variance_threshold'] * 1.1)
            self.adaptive_params['entropy_threshold'] = min(2.0, self.adaptive_params['entropy_threshold'] * 1.1)
            self.adaptive_params['margin_adjustment'] = max(0.3, self.adaptive_params['margin_adjustment'] * 0.8)
            
        elif feedback == 'perfect':
            # Piccoli aggiustamenti per mantenere la qualità
            pass
            
        # Aggiorna i parametri nel profilo
        if self.samurai_profiles and 'samurai_profiles' in self.samurai_profiles:
            profile = self.samurai_profiles['samurai_profiles'].get(self.active_profile)
            if profile:
                profile['adaptive_params'] = self.adaptive_params.copy()
    ...
    def extract_and_process_images(self, pdf_path: str, target_dpi: int = 300, max_pages: int = None, stop_callback=None, progress_callback=None) -> List[str]:
        ...
        try:
            ...
            for page_num in range(pages_to_process):
                # Controllo interruzione per ogni pagina
                if stop_callback and stop_callback():
                    logger.info("Elaborazione interrotta dall'utente durante l'estrazione immagini")
                    break
                
                # Aggiorna progresso per ogni pagina
                if progress_callback and pages_to_process > 0:
                    page_progress = 30 + (page_num / pages_to_process) * 30  # Da 30% a 60%
                    progress_callback(page_progress, f"Elaborazione pagina {page_num+1}/{pages_to_process}...")
                ...
                if image_list:
                    for img_index, img in enumerate(image_list):
                        # Controllo interruzione per ogni immagine
                        if stop_callback and stop_callback():
                            logger.info("Elaborazione interrotta dall'utente durante l'estrazione immagini")
                            break
                        xref = img[0]
    ...
    def analyze_image_ratio(self, image_path: str) -> Dict[str, str]:
        """Analizza il ratio di un'immagine e restituisce informazioni dettagliate"""
        try:
            detected_ratio = self.detect_image_ratio(image_path)
            optimal_format = self.get_optimal_format_for_ratio(detected_ratio)
            
            # Ottieni dimensioni effettive
            with Image.open(image_path) as img:
                width, height = img.size
                actual_ratio = max(width, height) / min(width, height)
                orientation = "landscape" if width > height else "portrait"
                
                # Rileva orientamento corretto del contenuto
                correct_orientation = self.detect_content_orientation(img)
                orientation_confidence = self.get_orientation_confidence(img)
            ...
            result = {
                ...
                "recommendation": f"Formato consigliato: {optimal_format} per ratio {detected_ratio}"
            }
            
            # Aggiungi informazioni sulla correzione dell'orientamento
            if correct_orientation != "correct":
                if orientation_confidence > 0.7:
                    result["recommendation"] += f" - CORREZIONE ORIENTAMENTO CONSIGLIATA: {correct_orientation} (confidenza: {orientation_confidence:.1%})"
                else:
                    result["recommendation"] += f" - Possibile problema orientamento: {correct_orientation} (confidenza bassa: {orientation_confidence:.1%})"
            
            return result
            
        except Exception as e:
            logger.error(f"Errore nell'analisi del ratio: {e}")
            return {
                "detected_ratio": "A4",
                "optimal_format": "A4",
                "actual_ratio": "unknown",
                "dimensions": "unknown",
                "orientation": "unknown",
                "correct_orientation": "unknown",
                "orientation_confidence": 0.0,
                "recommendation": "Errore nell'analisi, uso formato A4 di default"
            }
    
    def detect_and_crop_content(self, image_input, target_format: str = "A4", auto_detect_format: bool = False) -> Optional[str]:
        ...
        try:
            ...
            if isinstance(image_input, str):
                ...
                # Rilevamento automatico del formato se abilitato
                if auto_detect_format:
                    ratio_analysis = self.analyze_image_ratio(image_path)
                    suggested_format = ratio_analysis["optimal_format"]
                    logger.info(f"Rilevamento automatico: {ratio_analysis['recommendation']}")
                    if suggested_format != target_format:
                        logger.info(f"Cambio formato da {target_format} a {suggested_format} basato sul ratio rilevato")
                        target_format = suggested_format
                ...
                # Rilevamento automatico del formato se abilitato
                if auto_detect_format:
                    detected_ratio = self.detect_image_ratio(pil_image)
                    suggested_format = self.get_optimal_format_for_ratio(detected_ratio)
                    logger.info(f"Rilevamento automatico su immagine PIL: formato suggerito {suggested_format} per ratio {detected_ratio}")
                    if suggested_format != target_format:
                        logger.info(f"Cambio formato da {target_format} a {suggested_format} basato sul ratio rilevato")
                        target_format = suggested_format
                ...
                # Rilevamento automatico del formato se abilitato
                if auto_detect_format:
                    detected_ratio = self.detect_image_ratio(image_input)
                    suggested_format = self.get_optimal_format_for_ratio(detected_ratio)
                    logger.info(f"Rilevamento automatico su immagine PIL: formato suggerito {suggested_format} per ratio {detected_ratio}")
                    if suggested_format != target_format:
                        logger.info(f"Cambio formato da {target_format} a {suggested_format} basato sul ratio rilevato")
    ...
    def _detect_by_color_analysis(self, img: np.ndarray) -> Optional[Tuple[int, int, int, int]]:
        ...
        try:
            ...
            max_processing_time = 10  # Ridotto a 10 secondi per reattività
    ...
    def _detect_by_rectangular_contours(self, img: np.ndarray) -> Optional[Tuple[int, int, int, int]]:
        ...
        try:
            ...
            max_processing_time = 10  # Ridotto a 10 secondi
            ...
            if time.time() - start_time > max_processing_time:
                ...
                # Campionamento molto più aggressivo per grandi immagini
                if w > 6000 or h > 6000:
                    step = window_size  # Salta molto di più
            ...
            # Trova contorni (procediamo anche se timeout raggiunto, usando la maschera che abbiamo)
    ...
    def detect_image_ratio(self, image_input) -> str:
        """Rileva automaticamente il ratio di un'immagine e suggerisce il formato ottimale"""
        try:
            ...
            if isinstance(image_input, str):
                ...
                logger.error(f"Tipo di input non supportato: {type(image_input)}")
                return "A4"  # Fallback
            
            # Calcola il ratio (sempre >= 1, con orientamento)
            if width > height:
                ratio = width / height
            ...
            logger.info(f"Dimensioni rilevate: {width}x{height}, ratio: {ratio:.3f}, orientamento: {orientation}")
            
            # Definisce i ratio standard con tolleranza
            ratio_formats = {
                # Ratio quadrati
                1.0: "SQUARE",
                # Ratio rettangolari standard
                1.414: "A4",     # √2 ≈ 1.414 (A4, A3, A5)
                1.294: "LETTER",  # 8.5:11 ≈ 1.294
                1.545: "LEGAL",   # 8.5:14 ≈ 1.647
                1.333: "4:3",     # 4:3 ≈ 1.333
                1.5: "3:2",       # 3:2 = 1.5
                1.6: "16:10",     # 16:10 = 1.6
                1.778: "16:9",    # 16:9 ≈ 1.778
                2.0: "2:1",       # 2:1 = 2.0
                2.35: "CINEMA",   # Cinema scope ≈ 2.35
                3.0: "PANORAMIC" # Panoramico 3:1
            }
            
            # Trova il ratio più vicino con tolleranza del 5%
            best_match = "A4"  # Default fallback
            min_difference = float('inf')
            tolerance = 0.05  # 5% di tolleranza
            
            for standard_ratio, format_name in ratio_formats.items():
                difference = abs(ratio - standard_ratio)
                relative_difference = difference / standard_ratio
            ...
            if min_difference < float('inf'):
                accuracy = (1 - min_difference / ratio) * 100
                logger.info(f"Ratio riconosciuto: {best_match} (accuratezza: {accuracy:.1f}%, orientamento: {orientation})")
            else:
                logger.warning(f"Ratio non standard rilevato: {ratio:.3f}, uso formato A4")
                best_match = "A4"
            
            return best_match
            
        except Exception as e:
            logger.error(f"Errore nel rilevamento del ratio: {e}")
            return "A4"  # Fallback sicuro
    
    def get_optimal_format_for_ratio(self, detected_ratio: str, prefer_standard: bool = True) -> str:
        """Converte il ratio rilevato in un formato di stampa ottimale"""
        try:
            # Mappatura da ratio rilevati a formati di stampa
            ratio_to_format = {
                "SQUARE": "A4",      # I quadrati si adattano bene all'A4
                "A4": "A4",          # Perfetto match
                "LETTER": "LETTER",  # Perfetto match
                "LEGAL": "LEGAL",    # Perfetto match
                "4:3": "A4",         # Simile all'A4
                "3:2": "A4",         # Buon compromesso
                "16:10": "A4",       # Simile all'A4
                "16:9": "A3",        # Meglio su A3 per il formato wide
                "2:1": "A3",         # Formato wide, meglio A3
                "CINEMA": "A3",      # Ultra-wide, richiede A3
                "PANORAMIC": "A3"    # Panoramico, richiede A3
            }
            
            optimal_format = ratio_to_format.get(detected_ratio, "A4")
            
            if prefer_standard and optimal_format not in ["A4", "A3", "A5"]:
                # Se si preferiscono formati standard, converti a A4/A3
                if detected_ratio in ["16:9", "2:1", "CINEMA", "PANORAMIC"]:
                    optimal_format = "A3"
                else:
                    optimal_format = "A4"
            
            logger.info(f"Formato ottimale per ratio {detected_ratio}: {optimal_format}")
            return optimal_format
            
        except Exception as e:
            logger.error(f"Errore nella selezione del formato ottimale: {e}")
            return "A4"
    
    def detect_content_orientation(self, image: Image.Image) -> str:
        """Rileva l'orientamento corretto del contenuto dell'immagine analizzando testo e forme"""
        try:
            import cv2
            import numpy as np
            ...
            # Analizza l'immagine in 4 orientamenti (0°, 90°, 180°, 270°)
            orientations = {
                0: "correct",
                90: "rotate_270",  # Ruota di 270° per correggere
                180: "rotate_180",
                270: "rotate_90"   # Ruota di 90° per correggere
            }
            
            best_score = 0
            best_orientation = "correct"
            
            for angle, orientation_name in orientations.items():
                # Ruota l'immagine per il test
                if angle == 0:
                    test_image = gray
                else:
                    # Ruota l'immagine
                    ...
                    test_image = cv2.warpAffine(gray, rotation_matrix, (gray.shape[1], gray.shape[0]))
                
                # Calcola il punteggio per questo orientamento
                score = self._calculate_orientation_score(test_image)
                
                logger.info(f"Orientamento {angle}°: punteggio {score:.3f}")
                
                if score > best_score:
                    best_score = score
                    best_orientation = orientation_name
            
            logger.info(f"Orientamento migliore rilevato: {best_orientation} (punteggio: {best_score:.3f})")
    ...
    def get_orientation_confidence(self, image: Image.Image) -> float:
        """Calcola la confidenza del rilevamento dell'orientamento"""
        try:
            import cv2
            import numpy as np
            ...
            # Calcola punteggi per tutti gli orientamenti
            scores = []
            ...
            # La confidenza è la differenza tra il miglior punteggio e la media degli altri
            best_score = max(scores)
            other_scores = [s for s in scores if s != best_score]
            avg_other = np.mean(other_scores) if other_scores else 0
            
            confidence = min(1.0, (best_score - avg_other) / best_score if best_score > 0 else 0)
            
            logger.info(f"Confidenza orientamento: {confidence:.3f}")
            return confidence
            
        except Exception as e:
            logger.error(f"Errore nel calcolo confidenza orientamento: {e}")
            return 0.0
    
    def _calculate_orientation_score(self, gray_image: np.ndarray) -> float:
        """Calcola un punteggio per determinare quanto è corretto l'orientamento"""
        try:
            import cv2
            ...
            # 1. Analisi delle linee orizzontali (testo) - PESO MAGGIORE
            ...
            score += horizontal_score * 10.0  # Peso molto maggiore per le linee orizzontali
            
            # 2. Analisi OCR-like: rileva caratteri e parole
            # Applica soglia per evidenziare il testo
            _, thresh = cv2.threshold(gray_image, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
            
            # Trova contorni che potrebbero essere caratteri
            contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            
            # Analizza i contorni per caratteristiche tipiche del testo
            text_like_contours = 0
            horizontal_alignment = 0
            
            for contour in contours:
                area = cv2.contourArea(contour)
                if 50 < area < 5000:  # Dimensione tipica dei caratteri
                    x, y, w, h = cv2.boundingRect(contour)
                    aspect_ratio = w / h if h > 0 else 0
                    
                    # Caratteristiche tipiche del testo
                    if 0.2 < aspect_ratio < 3.0:  # Ratio tipico dei caratteri
                        text_like_contours += 1
                        
                        # Verifica allineamento orizzontale
                        if h < w:  # Carattere più largo che alto (orientamento corretto)
                            horizontal_alignment += 1
            
            if text_like_contours > 0:
                text_score = (horizontal_alignment / text_like_contours) * 5.0
                score += text_score
            
            # 3. Analisi della distribuzione del contenuto
            # Il contenuto dovrebbe essere più denso nella parte superiore
            top_quarter = gray_image[:height//4, :]
            middle_half = gray_image[height//4:3*height//4, :]
            bottom_quarter = gray_image[3*height//4:, :]
            
            top_density = np.std(top_quarter)
            middle_density = np.std(middle_half)
            bottom_density = np.std(bottom_quarter)
            
            # Documenti tipici hanno più contenuto in alto e al centro
            if top_density > bottom_density:
                score += 2.0
            if middle_density > bottom_density:
                score += 1.0
            
            # 4. Analisi dei margini
            # I margini dovrebbero essere più uniformi sui lati
            left_margin = gray_image[:, :width//10]
            right_margin = gray_image[:, 9*width//10:]
            top_margin = gray_image[:height//10, :]
            bottom_margin = gray_image[9*height//10:, :]
            
            # Margini dovrebbero essere più uniformi (meno varianza)
            margin_uniformity = 0
            for margin in [left_margin, right_margin, top_margin, bottom_margin]:
                if margin.size > 0:
                    uniformity = 1.0 / (1.0 + np.std(margin))
                    margin_uniformity += uniformity
            
            score += margin_uniformity * 0.5
            
            # 5. Analisi della direzione principale del gradiente
            # Il testo ha gradienti principalmente verticali (per caratteri normali)
            sobelx = cv2.Sobel(gray_image, cv2.CV_64F, 1, 0, ksize=3)
            sobely = cv2.Sobel(gray_image, cv2.CV_64F, 0, 1, ksize=3)
            
            # Calcola la direzione predominante del gradiente
            gradient_magnitude = np.sqrt(sobelx**2 + sobely**2)
            strong_gradients = gradient_magnitude > np.percentile(gradient_magnitude, 90)
            
            if np.sum(strong_gradients) > 0:
                vertical_gradients = np.sum(np.abs(sobely[strong_gradients]) > np.abs(sobelx[strong_gradients]))
                vertical_ratio = vertical_gradients / np.sum(strong_gradients)
                score += vertical_ratio * 2.0
            
            return score
            
        except Exception as e:
            logger.error(f"Errore nel calcolo punteggio orientamento: {e}")
            return 0.0
    
    def auto_correct_orientation(self, image: Image.Image, min_confidence: float = 0.7) -> tuple[Image.Image, dict]:
        """Corregge automaticamente l'orientamento dell'immagine se necessario
        
        Args:
            image: Immagine PIL da correggere
            min_confidence: Confidenza minima per applicare la correzione (default: 0.7)
            
        Returns:
            tuple: (immagine_corretta, info_correzione)
        """
        try:
            # Rileva orientamento
            correct_orientation = self.detect_content_orientation(image)
            orientation_confidence = self.get_orientation_confidence(image)
            
            correction_info = {
                "original_orientation": correct_orientation,
                "confidence": orientation_confidence,
                "corrected": False,
                "rotation_applied": 0
            }
            
            # Applica correzione se necessario e confidenza è sufficiente
            if correct_orientation != "correct" and orientation_confidence >= min_confidence:
                logger.info(f"Applicazione correzione automatica orientamento: {correct_orientation} (confidenza: {orientation_confidence:.3f})")
                
                corrected_image = image.copy()
                
                if correct_orientation == "rotate_90":
                    corrected_image = corrected_image.rotate(-90, expand=True)
                    correction_info["rotation_applied"] = -90
                elif correct_orientation == "rotate_180":
                    corrected_image = corrected_image.rotate(180, expand=True)
                    correction_info["rotation_applied"] = 180
                elif correct_orientation == "rotate_270":
                    corrected_image = corrected_image.rotate(90, expand=True)
                    correction_info["rotation_applied"] = 90
                
                correction_info["corrected"] = True
                logger.info(f"Orientamento corretto applicando rotazione di {correction_info['rotation_applied']}°")
                
                return corrected_image, correction_info
            else:
                if correct_orientation != "correct":
                    logger.info(f"Orientamento potenzialmente errato ({correct_orientation}) ma confidenza troppo bassa ({orientation_confidence:.3f} < {min_confidence})")
                
                return image, correction_info
                
        except Exception as e:
            logger.error(f"Errore nella correzione automatica orientamento: {e}")
            correction_info = {
    ...
    def _resize_to_target_format(self, img: np.ndarray, target_format: str = "A4", target_dpi: int = 300, auto_detect_ratio: bool = True) -> np.ndarray:
        """Ridimensiona l'immagine per massimizzare l'utilizzo del campo disponibile mantenendo proporzioni e qualità"""
        try:
            # Rilevamento automatico del ratio se abilitato
            if auto_detect_ratio:
                detected_ratio = self.detect_image_ratio(img)
                suggested_format = self.get_optimal_format_for_ratio(detected_ratio)
                
                # Se il formato suggerito è diverso da quello richiesto, informa l'utente
                if suggested_format != target_format:
                    logger.info(f"Formato suggerito dal rilevamento automatico: {suggested_format} (richiesto: {target_format})")
                    # Opzionalmente, potresti usare il formato suggerito:
                    # target_format = suggested_format
            ...
            format_dimensions = {
                ...
                "SQUARE": (2480, 2480)   # Formato quadrato basato su A4
            }
            ...
            if final_width < target_width or final_height < target_height:
                ...
                # Calcola coordinate sorgente (resized) e destinazione (canvas)
                ...
                # Calcola dimensioni da copiare
                h_copy = min(final_height - src_y1, target_height - dst_y1)
                w_copy = min(final_width - src_x1, target_width - dst_x1)
                
                if h_copy > 0 and w_copy > 0:
                    canvas[dst_y1:dst_y1+h_copy, dst_x1:dst_x1+w_copy] = resized[src_y1:src_y1+h_copy, src_x1:src_x1+w_copy]
                    resized = canvas
                    logger.info(f"Immagine centrata e adattata in canvas {target_width}x{target_height}")
    ...
    def process_pdf_file(self, pdf_path: str, target_dpi: int = 300, crop_content: bool = True, target_format: str = "A4", max_pages: int = None, stop_callback=None, progress_callback=None) -> Dict:
        """Processa un singolo file PDF"""
        logger.info(f"Inizio elaborazione: {pdf_path}")
        
        # Sistema Samurai v3.0: Rilevamento automatico tipo documento
        pdf_name = os.path.basename(pdf_path)
        if self.samurai_profiles and self.samurai_profiles.get('auto_detection', True):
            detected_profile = self._detect_document_type(pdf_name)
            if detected_profile != self.active_profile:
                logger.info(f"Rilevato documento di tipo {detected_profile}, cambio profilo da {self.active_profile}")
                self.set_active_profile(detected_profile)
        ...
        result = {
            ...
            'cropped_images': [],
            'errors': [],
            'samurai_profile_used': self.active_profile  # v3.0: traccia profilo utilizzato
        }
        ...
        try:
            ...
            if stop_callback and stop_callback():
                ...
                result['errors'].append("Elaborazione interrotta dall'utente")
                return result
            
            # Estrae metadati
            if progress_callback:
                progress_callback(10, "Estrazione metadati...")
            result['metadata'] = self.extract_pdf_metadata(pdf_path, max_pages=max_pages)
            ...
            if progress_callback:
                progress_callback(30, "Estrazione immagini...")
            output_images = self.extract_and_process_images(pdf_path, target_dpi, max_pages, stop_callback, progress_callback)
            ...
            if crop_content:
                if progress_callback:
                    progress_callback(60, "Auto-cropping in corso...")
                cropped_images = []
                total_images = len(output_images)
                for idx, item in enumerate(output_images):
                    # Controllo interruzione per ogni immagine
                    if stop_callback and stop_callback():
                        logger.info("Elaborazione interrotta dall'utente durante il ritaglio")
                        result['errors'].append("Elaborazione interrotta dall'utente durante il ritaglio")
                        return result
                    
                    # Aggiorna progresso per ogni immagine ritagliata
                    if progress_callback and total_images > 0:
                        crop_progress = 60 + (idx / total_images) * 30  # Da 60% a 90%
                        progress_callback(crop_progress, f"Ritaglio immagine {idx+1}/{total_images}...")
...
def test_ratio_recognition():
    """Funzione di test per il rilevamento automatico del ratio"""
    processor = KatanaProcessor(auto_ratio_detection=True)
    
    print("=== TEST RILEVAMENTO AUTOMATICO RATIO ===")
    print(f"Rilevamento automatico: {'ATTIVO' if processor.is_auto_ratio_detection_enabled() else 'DISATTIVO'}")
    
    # Test con immagini di esempio (se disponibili)
    test_images = ["test_a4.jpg", "test_a3.jpg", "test_letter.jpg"]
    
    for test_image in test_images:
        if os.path.exists(test_image):
            try:
                ratio_info = processor.analyze_image_ratio(test_image)
                print(f"\n📄 {test_image}:")
                print(f"   Dimensioni: {ratio_info['dimensions']}")
                print(f"   Ratio rilevato: {ratio_info['detected_ratio']}")
                print(f"   Formato ottimale: {ratio_info['optimal_format']}")
                print(f"   Orientamento: {ratio_info['orientation']}")
                print(f"   Raccomandazione: {ratio_info['recommendation']}")
            except Exception as e:
                print(f"❌ Errore nell'analisi di {test_image}: {e}")
        else:
            print(f"⚠️  File di test {test_image} non trovato")
    
    # Test controlli
    print("\n=== TEST CONTROLLI ===")
    processor.disable_auto_ratio_detection()
    print(f"Dopo disabilitazione: {'ATTIVO' if processor.is_auto_ratio_detection_enabled() else 'DISATTIVO'}")
    
    processor.enable_auto_ratio_detection()
    print(f"Dopo riabilitazione: {'ATTIVO' if processor.is_auto_ratio_detection_enabled() else 'DISATTIVO'}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        print("🎌 Katana - Agente di Elaborazione PDF 🎌")
        print("\nUtilizzo:")
        print("  python katana.py <input_file_or_dir> [opzioni]")
        print("\nOpzioni:")
        print("  --output-dir DIR   Directory di output (default: output_images)")
        print("  --dpi DPI          DPI target (default: 300)")
        print("  --no-crop          Disabilita ritaglio automatico")
        print("  --format FMT       Formato target (A4, A3, LETTER, LEGAL)")
        print("  --test-ratio       Esegui test rilevamento ratio")
        print("\nEsempio:")
        print("  python katana.py documento.pdf --format A4")
        sys.exit(0)
        
    # Gestione flag di test speciale
    if "--test-ratio" in sys.argv:
        print("🎌 Katana - Test Rilevamento Automatico Ratio 🎌")
        test_ratio_recognition()
        sys.exit(0)
        
    # Altrimenti esegui il main normale
    sys.exit(main())
